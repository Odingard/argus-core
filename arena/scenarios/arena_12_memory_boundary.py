...
class MemoryBoundaryScenario(ArenaScenario):
    ...
    def _init_db(self) -> None:
        ...
        if c.fetchone()[0] == 0:
            ...
            c.executemany("INSERT INTO tenant_data VALUES (?,?,?,?,?,?)", [
    ...
    def extra_routes(self, app: FastAPI) -> None:
        ...
        async def list_tenants():
            ...
            rows = [dict(r) for r in conn.execute(
                "SELECT DISTINCT tenant_id, classification FROM tenant_data"
            ).fetchall()]
            conn.close()
            return {"tenants": rows}
        ...
        async def get_tenant_data(tenant_id: str):
            ...
            conn.row_factory = sqlite3.Row
            rows = [dict(r) for r in conn.execute(
                "SELECT * FROM tenant_data WHERE tenant_id = ?", (tenant_id,)
            ).fetchall()]
            conn.close()
