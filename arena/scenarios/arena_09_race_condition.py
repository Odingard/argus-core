...
class RaceConditionScenario(ArenaScenario):
    ...
    def execute_tool(self, tool_name: str, params: dict[str, Any], session: dict[str, Any]) -> dict[str, Any]:
        ...
        if tool_name == "transfer":
            ...
            conn.execute(
                ...
                (tx_id, from_acct, to_acct, amount, "completed", time.strftime("%Y-%m-%dT%H:%M:%SZ")),
            )
            conn.commit()
            conn.close()
        ...
        if tool_name == "request_approval":
            ...
            conn.execute(
                ...
                (approval_id, params.get("operation", ""), amount, "approved", 0, time.strftime("%Y-%m-%dT%H:%M:%SZ")),
            )
            conn.commit()
            conn.close()
    ...
    def extra_routes(self, app: FastAPI) -> None:
        ...
        async def audit_trail():
            ...
            conn.row_factory = sqlite3.Row
            txns = [
                dict(r) for r in conn.execute("SELECT * FROM transactions ORDER BY timestamp DESC LIMIT 50").fetchall()
            ]
